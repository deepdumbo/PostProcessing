...
number_of_projections=hdr.encoding.encodingLimits.kspace_encoding_step_1.maximum -hdr.encoding.encodingLimits.kspace_encoding_step_1.minimum +1 ;
...
number_of_channels_after_pca=8;
...
threshold.fin_all=number_of_projections*number_of_repetitions-threshold.exclusion_all;

threshold.exclusion_ecg=1000;
threshold.debut_ecg=threshold.exclusion_ecg+1;
threshold.fin_ecg=number_of_projections*number_of_repetitions-threshold.exclusion_ecg/2;


%% chargement des données de self gating

data.input.raw=zeros(number_of_projections,number_of_channels,number_of_repetitions);

for r=1:1:number_of_repetitions
...
[ data.filter.resp ] = reformat_to_each_repitition( data.allrep.filter.resp, number_of_repetitions );

[ data.substract.resp ] = reformat_to_each_repitition( data.allrep.substract.resp, number_of_repetitions );

%% ici on boucle sur les repetitions

[ navigator ] = extract_navigator_resp_and_ecg( data , threshold.debut, threshold.fin ,number_of_repetitions , threshold , Fs_ute );
...
number_of_output_respiratory_phases=3; % parametre qui doit venir de la special card 
...
number_of_output_cardiac_phases=20; %g.NumberofPhase  % parametre qui doit venir de la special card 
...
input.matrice=zeros(readout, number_of_projections ,number_of_channels_after_pca, repetition_for_reco);
...
for r=1:1:repetition_for_reco
    ...
    [ tempo ] =single( read_binary_complex( filename, [ readout*number_of_channels_after_pca* number_of_projections ] ));
    
    tempo1=reshape(tempo, [readout, number_of_channels_after_pca,  number_of_projections]);
...
for repetition_for_reco_indice=10:1:repetition_for_reco;
 ...
 if (strcmp(mode.save_kspace_sbubset,'Y'))
    ...
    for j=1:1:number_of_output_respiratory_phases
        for i=1:number_of_output_cardiac_phases
            ...
            trajectory=trajectory_subsets{i,j};
            kspace=kspace_subsets{i,j};
            
            save(filename_traj, 'trajectory' );
            save(filename_kspace, 'kspace');
...
for ll=74:2:74
    for jj=1:1:number_of_output_respiratory_phases
        for ii=1:1:number_of_output_cardiac_phases
            subplot(number_of_output_respiratory_phases,number_of_output_cardiac_phases,ii+ (number_of_output_cardiac_phases)*(jj-1));
            
            str_msg=sprintf('%d %d %d %d \n',ii,jj,ii+ (number_of_output_cardiac_phases-1)*jj ,jj); disp(str_msg);
...
while(1)
    ...
    for ii=1:1:number_of_output_cardiac_phases
        for jj=1:1:1
            ...
            %               for kk=indice:10:indice+20
            
            %                 kk=50;
            %                 subplot(number_of_output_respiratory_phases,4, kk-49 +(jj-1)*4);
            %                 temp=squeeze(image_crop(:,kk,:,ii,jj));
            %                 imagesc(imrotate(temp,90));  title( int2str(kk)); colormap(gray);
            
            kk=65;
            subplot(number_of_output_respiratory_phases,taille, kk-64 +(jj-1)*taille);
            temp=squeeze(image_crop(kk,:,:,ii,jj));
            imagesc(imrotate(temp,90));  title( int2str(kk)); colormap(gray);
            
            kk=75;
            subplot(number_of_output_respiratory_phases,taille, kk-74+1 +(jj-1)*taille);
            temp=squeeze(image_crop(:,kk,:,ii,jj));
            imagesc(imrotate(temp,90));  title( int2str(kk)); colormap(gray);
            
            kk=90;
            subplot(number_of_output_respiratory_phases,taille, kk-89+2 +(jj-1)*taille);
            temp=squeeze(image_crop(:,kk,:,ii,jj));
            imagesc(imrotate(temp,90));  title( int2str(kk)); colormap(gray);
            
            kk=65;
            subplot(number_of_output_respiratory_phases,taille, kk-64 +(2-1)*taille);
            temp=squeeze(A.image_crop_save_data_ok(kk,:,:,ii,jj));
            imagesc(imrotate(temp,90));  title( int2str(kk)); colormap(gray);
            
            kk=75;
            subplot(number_of_output_respiratory_phases,taille, kk-74+1 +(2-1)*taille);
            temp=squeeze(A.image_crop_save_data_ok(:,kk,:,ii,jj));
            imagesc(imrotate(temp,90));  title( int2str(kk)); colormap(gray);
            
            kk=90;
            subplot(number_of_output_respiratory_phases,taille, kk-89+2 +(2-1)*taille);
            temp=squeeze(A.image_crop_save_data_ok(:,kk,:,ii,jj));
            imagesc(imrotate(temp,90));  title( int2str(kk)); colormap(gray);
            %             end
