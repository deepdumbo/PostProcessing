...
for r=1:1:repetition
    ...
    filename=[folder_data,'/Navigator/dense_data_store_abs_',str_r,'.dat']
    temp=load(filename);
...
nb_phase_respiratoire=1;

[ table ] = extract_liste_respiratory( single(navigator.allrep.resp.normalize), threshold.debut_all, threshold.fin_all, nb_phase_respiratoire );
...
nb_phase_ecg=20 %g.NumberofPhase    %NB of phases

[ table_ecg_self_gating ] = extract_list_ecg( single(data_peak_dectection.moins_new(:,1)), nb_phase_ecg ,repetition_for_reco, nbProjections  );

debut_change=5;

[ table_ecg_self_gating_in_systole ] = start_list_ecg_in_systole( table_ecg_self_gating , debut_change, nb_phase_ecg);
...
t_ecg = single((0:L_ecg-1)*T_ecg);        % Time vector
...
t_ute = single((0:L_ute-1)*T_ute);        % Time vector
...
% datatosave(:,2)=-ecg_truth(:,2)/50;
%
% min(datatosave(:,2))
% max(datatosave(:,2))
%
% save('/home/valery/Reseau/Valery/MatlabUnix/GPU/MakeFigures/ecg_voie1.txt','datatosave','-ascii');
%
% clear  datatosave
% datatosave(:,1)=t_ecg(:,1);
...
% hold on;
% plot(t_ute, table_ecg_self_gating*5-200,'g');
% hold on;
% plot(t_ute, table_ecg_recording*5-300,'b');
hold on;
plot(t_ute, table_ecg_recording*5-300,'r');
hold on;
plot(t_ute, table_ecg_self_gating_in_systole*5-300,'g');
hold on;
% plot(t_ute, table_ecg_recording_extended-160,'b');
...
% image_crop=single(zeros(info.N/2,info.N/2,info.N/2,nb_phase_ecg, nb_phase_respiratoire));
% image_nocrop=single(zeros(info.N,info.N,info.N));

clear rawDataECG trajECG FT mattest DCF matrice_nRep traj_nRep image_tempo_1 temp ktrajIdxAvant ktrajIdxApres ktrajIdx image_nocrop image_crop

clear rawDataECG trajECG rawDataECG_avant   trajECG_avant   rawDataECG_apres   trajECG_apres
...
for repetition_for_reco_indice=10:1:repetition_for_reco;
    ...
    if (strcmp(mode.filter,'Y'))
        ...
        rawDataECG_avant=cell(nb_phase_ecg,nb_phase_respiratoire,3);
        trajECG_avant=cell(nb_phase_ecg,nb_phase_respiratoire,3);
        
        rawDataECG_apres=cell(nb_phase_ecg,nb_phase_respiratoire,3);
        trajECG_apres=cell(nb_phase_ecg,nb_phase_respiratoire,3);
    ...
    for j=1:1:nb_phase_respiratoire
        ...
        for i=1:nb_phase_ecg
            ...
            [ table_input_ecg ] = remove_kspace_from_other_respiratory_stage(table.respiration.liste_ready(1:(nbProjections*repetition_for_reco_indice)) , j,  table_ecg_for_reco );
            ...
            if (i<=1)
                index_corrige_avant=nb_phase_ecg+1;
                index_corrige_apres=i;
                
            elseif (i>=nb_phase_ecg)
                index_corrige_avant=i;
                index_corrige_apres=0;
            else
                index_corrige_avant=i;
                index_corrige_apres=i;
            end
            
            str_msg=sprintf(' ------------------------ '); disp(str_msg);
            str_msg=sprintf(' phase resp %d  phase ecg  %d ,  nb projections %d', j, i, size(ktrajIdx,1)); disp(str_msg);
            
            if (strcmp(mode.filter,'Y'))
                
                %% on met a zero les projections qui ne correspondent pas à la bonne phase respiratoire
                [ table_input_ecg_ext ] = remove_kspace_from_other_respiratory_stage(table.respiration.liste_ready(1:(nbProjections*repetition_for_reco_indice)) , j,  table_ecg_recording_extended );
                
                clear  ktrajIdxAvant ktrajIdxApres
                
                %             for ll=1:1:taille_filtre
                for ll=1:1:taille_filtre
                    %table_ecg_recording_extended
                    new_index=(index_corrige_avant-2)*taille_filtre;
                    ktrajIdxAvant=find(table_input_ecg_ext(:)==(new_index+ll));
                    str_msg=sprintf(' avant   %d %d je mange %d soit %d projections quantité %d ', i, new_index ,new_index +ll,size(ktrajIdxAvant,1), 1-ll/keep_filtre) ;disp(str_msg);
                    
                    trajECG_avant{i,j,ll}=traj_nRep(end*(1-ll/keep_filtre):end ,ktrajIdxAvant,:);
                    rawDataECG_avant{i,j,ll}=matrice_nRep(end*(1-ll/keep_filtre):end,ktrajIdxAvant,:);
                    
                    %                 trajECG_avant{i,j,ll}=traj_nRep(: ,ktrajIdxAvant,:);
                    %                 rawDataECG_avant{i,j,ll}=matrice_nRep(:,ktrajIdxAvant,:);
                end
                
                
                for ll=1:1:taille_filtre
                    new_index=(index_corrige_apres)*taille_filtre;
                    ktrajIdxApres=find(table_input_ecg_ext(:)==(new_index+ll));
                    str_msg=sprintf(' apres   %d %d je mange %d soit %d projections  quantité %d', i, new_index ,new_index+ll,size(ktrajIdxApres,1), ll/keep_filtre ) ;disp(str_msg);
                    
                    trajECG_apres{i,j,ll}=traj_nRep(end*(ll/keep_filtre):end ,ktrajIdxApres,:);
                    rawDataECG_apres{i,j,ll}=matrice_nRep(end*(ll/keep_filtre):end,ktrajIdxApres,:);
                    
                    %                 trajECG_apres{i,j,ll}=traj_nRep(: ,ktrajIdxApres,:);
                    %                 rawDataECG_apres{i,j,ll}=matrice_nRep(:,ktrajIdxApres,:);
    ...
    %% concatenation des vecteurs du filtre
    ...
    if (strcmp(mode.filter,'Y'))
        ...
        for j=1:1:nb_phase_respiratoire
            ...
            for i=1:nb_phase_ecg
                ...
                trajECG{i,j}=reshape(trajECG{i,j},[],3);
                
                rawDataECG{i,j}=reshape(rawDataECG{i,j},[],8);
                
                for ll=1:1:taille_filtre
                    
                    trajECG{i,j}=[trajECG{i,j};   reshape(trajECG_avant{i,j,ll},[],3)];
                    trajECG{i,j}=[trajECG{i,j};   reshape(trajECG_apres{i,j,ll},[],3)];
                    
                    rawDataECG{i,j}=[rawDataECG{i,j};   reshape(rawDataECG_avant{i,j,ll},[],8)];
                    rawDataECG{i,j}=[rawDataECG{i,j};   reshape(rawDataECG_apres{i,j,ll},[],8)];
                    
                    str_msg=sprintf('complete with sliding window %d %d %d %d %d %d %d', i, j , ll, size(trajECG{i,j},1), size(trajECG{i,j},2) , size(rawDataECG{i,j},1) , size(rawDataECG{i,j},2)) ;disp(str_msg);
    ...
    clear trajECG_avant rawDataECG_apres trajECG_avant trajECG_apres
    ...
    for j=1:1:nb_phase_respiratoire
        for i=1:nb_phase_ecg
            ...
            info.numIter = 1;
            info.effMtx  = enc_Nx;
            DCF = sdc3_MAT(reshape(trajECG{i,j},[],3)',info.numIter,info.effMtx,info.verbose,info.osf);
            ...
            image_nocrop(:,:,:) = coil_combine( rawDataECG{i,j} , FT, info.N);
...
for ii=50:2:size(image_crop,2)-50
    for jj=1:1:1
        ...
        subplot(2,3,jj+3)
        temp=squeeze(A.image_crop_save_data_ok(:,ii,:,jj));
        imagesc(imrotate(temp,90));  title( int2str(ii)); colormap(gray);
    end
    pause(0.5)
end
...
%             kk=70;
%             subplot(nb_phase_respiratoire,taille, kk-69 +(2-1)*taille);
%             temp=squeeze(A.image_crop_save_data_ok(kk,:,:,ii,jj));
%             imagesc(imrotate(temp,90));  title( int2str(kk)); colormap(gray);
%             
%             kk=80;
%             subplot(nb_phase_respiratoire,taille, kk-79+1 +(2-1)*taille);
%             temp=squeeze(A.image_crop_save_data_ok(:,kk,:,ii,jj));
%             imagesc(imrotate(temp,90));  title( int2str(kk)); colormap(gray);
%             
%             kk=90;
%             subplot(nb_phase_respiratoire,taille, kk-89+2 +(2-1)*taille);
%             temp=squeeze(A.image_crop_save_data_ok(:,kk,:,ii,jj));
%             imagesc(imrotate(temp,90));  title( int2str(kk)); colormap(gray);
            %             end
