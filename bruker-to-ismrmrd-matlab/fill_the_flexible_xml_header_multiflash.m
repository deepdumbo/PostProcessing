function [ header ] = fill_the_flexible_xml_header_multiflash( ex, i )
...
header.experimentalConditions.H1resonanceFrequency_Hz = ex1.method.PVM_FrqRef(1)*1e6; % 9.4T
...
header.acquisitionSystemInformation.systemVendor = ex1.acqp.ORIGIN{1};
header.acquisitionSystemInformation.systemModel = ex1.acqp.ACQ_station{1};
header.acquisitionSystemInformation.systemFieldStrength_T= ex1.acqp.BF1/42.577;
header.acquisitionSystemInformation.receiverChannels = ex1.method.PVM_EncNReceivers;
header.acquisitionSystemInformation.relativeReceiverNoiseBandwidth=1;
header.acquisitionSystemInformation.institutionName =ex1.acqp.ACQ_institution{1};
header.acquisitionSystemInformation.stationName =ex1.acqp.ACQ_station{1};
 

fprintf('-------------------------------------------------------------- \n');
fprintf('%s\n', ex1.method.Method{1,1});
fprintf('-------------------------------------------------------------- \n');
...
if (ex1.method.PVM_EncPft(1)~=1)
    disp('attention partial fourier suivant x')
end

if (ex1.method.PVM_EncPft(2)~=1)
...
header.encoding.encodingLimits.contrast.maximum = 2;
...
header.sequenceParameters.TE(i) = ex.acqp.ACQ_echo_time;
