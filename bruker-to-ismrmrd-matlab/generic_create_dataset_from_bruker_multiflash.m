...
%         output_tmp          = ['/home/', str_user, '/Dicom/DIXON/Validation/RecoData/Ex_Vivo/2D/No_Grappa/20180216/',num2str(n_reco(numb))];
    ...
    %     output_tmp          = ['/home/', str_user, '/Dicom/DIXON/Validation/RecoData/In_Vitro/2D/No_Grappa/20180227/',num2str(n_reco(numb))];
          acquisition_path    = ['/home/', str_user, '/mount/Imagerie/For_Kylian/Dixon/Verification/Ex_Vivo/2D/20180322/',num2str(n_reco(numb))];
          output_tmp          = ['/home/', str_user, '/Dicom/DIXON/Verification/Ex_Vivo/2D/20180322/',num2str(n_reco(numb))];
    ...
    [header(numb)] = fill_the_flexible_xml_header_multiflash(ex, numb);
    ...
    [ idx(numb) , flag(numb) ] = fill_the_idx_multiflash( header(numb) , ex);

    clear  check_indice

    for i = 1:size(idx(numb).kspace_encode_step_1,2)
        if (idx(numb).contrast(i)==0)
            check_indice(idx(numb).kspace_encode_step_1(i)+1,idx(numb).kspace_encode_step_2(i)+1,1)=1;
        end
    end
...
%     subplot(2,1,1); imagesc(check_indice(:,:,1));
    ...
    acqblock(numb) = ismrmrd.Acquisition(number_of_lines);

    % Set the header elements that don't change
    acqblock(numb).head.version(:) = 1;
    acqblock(numb).head.number_of_samples(:) = readout;
    acqblock(numb).head.center_sample(:) = floor(readout/2);
    acqblock(numb).head.active_channels(:) = number_of_channels;
    acqblock(numb).head.read_dir  = repmat([1 0 0]',[1 number_of_lines]);
    acqblock(numb).head.phase_dir = repmat([0 1 0]',[1 number_of_lines]);
    acqblock(numb).head.slice_dir = repmat([0 0 1]',[1 number_of_lines]);
    ...
    for acqno = 1:number_of_lines
        ...
        % c-style counting
        acqblock(numb).head.scan_counter(acqno) =  acqno-1;
        acqblock(numb).head.idx.kspace_encode_step_1(acqno) = idx(numb).kspace_encode_step_1(acqno);
        acqblock(numb).head.idx.kspace_encode_step_2(acqno) = idx(numb).kspace_encode_step_2(acqno);
        acqblock(numb).head.idx.repetition(acqno) = idx(numb).repetition(acqno);
        acqblock(numb).head.idx.slice(acqno) = idx(numb).slice(acqno);
        acqblock(numb).head.idx.set(acqno) = idx(numb).set(acqno);
        acqblock(numb).head.idx.contrast(acqno) = idx(numb).contrast(acqno);
        acqblock(numb).head.sample_time_us(acqno)=1/ex.acqp.SW_h*1e6;
