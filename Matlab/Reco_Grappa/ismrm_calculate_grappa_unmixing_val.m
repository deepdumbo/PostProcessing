function [unmix, gmap] = ismrm_calculate_grappa_unmixing_val(source_data, kernel_size, acc_factor, data_mask, csm, target_data, verbose, coil_combine_mode)
%
...
%data_mask indique la positon des ligne de reference
...
if (isempty(csm)),
    ...
    if(coil_combine_mode==1)
        csm = ismrm_estimate_csm_walsh(csm); %Estimate coil sensitivity maps.
    elseif (coil_combine_mode==2)
        csm = calculate_csm_walsh( csm, 5, 3 ); %Estimate coil sensitivity maps.
    elseif (coil_combine_mode==3)
       [ csm ] = coil_map_study_2d_Inati( ifft_2D(source_data), 5, 3 );
    elseif (coil_combine_mode==4) 
         csm = calculate_csm_inati_iter( csm, 5, 5, 1e-3 ); %Estimate coil sensitivity maps.   
    elseif (coil_combine_mode==5)        
        [  csm_mckenzie ]= ismrm_estimate_csm_mckenzie( ifft_2D(source_data));        
    elseif (coil_combine_mode==6)      
        % code pour debug
        for c = 1:16
            str_c=sprintf('%d',c-1);
            filename=['/home/valery/Tempo/coil_map_',str_c,'.bin'];
            [ tempo_gt ] = read_binary_complex( filename, size(data_mask,1)*size(data_mask,1));
            coil_map(:,:,c)=reshape(tempo_gt, [size(data_mask,1) , size(data_mask,1)]);
        end        
        csm=coil_map;    
    else
        csm = ismrm_estimate_csm_walsh(csm); %Estimate coil sensitivity maps.
